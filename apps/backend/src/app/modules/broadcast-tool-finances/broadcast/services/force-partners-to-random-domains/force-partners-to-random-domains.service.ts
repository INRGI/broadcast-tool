...
export class ForcePartnersToRandomDomainsService {
  ...
  public async execute(
    ...
    if (
      broadcastRules.usageRules?.partnerMinTabLimit &&
      broadcastRules.usageRules?.partnerMaxTabLimit.length > 0
    ) {
      const partnersMinTabLimit = broadcastRules.usageRules?.partnerMinTabLimit;

      for (const date of dateRange) {
        let sentCount = 0;
        for (const { partnerName, limit, sheetName } of partnersMinTabLimit) {
          sentCount = 0;
          ...
          while (sentCount < limit) {
            ...
            const allDomains = broadcast.sheets.find(
            ...
            for (const domain of shuffledDomains) {
              ...
              if (
                ...
                !domain.broadcastCopies.some(
                  (d) =>
                    d.date === date &&
                    d.copies.some((c) =>
                      c.copyType.includes(CopyType.Conversion)
                    )
                )
              ) {
                continue;
              ...
              if (alreadyExists) {
                copyIndex =
                  (copyIndex + 1) % convertibleCopiesForPartner.length;
                continue;
              }
